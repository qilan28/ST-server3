...
### 2. 实例启动成功但立即停止（状态显示为stopped）

这是最常见的问题之一，通常表现为：
- 启动日志显示成功，但状态检查为 "stopped"
- 进程ID为0
- 实例无法访问

**可能原因：**
1. SillyTavern 启动参数错误
2. 依赖模块缺失或版本不兼容  
3. 端口冲突或权限问题
4. SillyTavern 配置文件错误
5. Node.js 版本过低或过高
...
**一键修复（推荐）：**
1. 运行 `quick-fix-instances.bat` 批处理文件
2. 等待自动修复和重启完成
...
对于复杂的启动问题，我们提供了详细的诊断工具：

```bash
node diagnose-instance.js <用户名>
```

例如：
```bash
node diagnose-instance.js 123456
```
